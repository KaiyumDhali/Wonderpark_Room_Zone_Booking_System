...
public function create2($roomId, $date)
{
...
public function roomBookingSearch($id, $startDate, $endDate)
{
    ...
    $bookings = DB::table('bookings')
        ...
        ->where(function ($q) use ($startDate, $endDate) {
            ...
            $q->where(function($q2) use ($startDate, $endDate) {
                $q2->whereNotNull('check_in_datetime')
                   ->whereNotNull('check_out_datetime')
                   ->where(function($q3) use ($startDate, $endDate) {
                        $q3->whereBetween('check_in_datetime', [$startDate.' 00:00:00', $endDate.' 23:59:59'])
                           ->orWhereBetween('check_out_datetime', [$startDate.' 00:00:00', $endDate.' 23:59:59']);
                   });
            })
            // day-wise bookings
            ->orWhere(function($q2) use ($startDate, $endDate) {
                $q2->whereNull('check_in_datetime')
    ...
    foreach ($dates as $date) {
        ...
        $isBooked = $bookings->contains(function($b) use ($dayStart, $dayEnd) {
            // hourly booking
            if ($b->check_in_datetime && $b->check_out_datetime) {
                $start = strtotime($b->check_in_datetime);
                $end = strtotime($b->check_out_datetime);

                // mark booked **only if booking overlaps entire day**
                $fullDay = ($start <= $dayStart && $end >= $dayEnd);
                return $fullDay;
            }

            // day-wise booking
            if ($b->check_in_date && $b->check_out_date) {
                return strtotime($b->check_in_date) <= $dayEnd && strtotime($b->check_out_date) > $dayStart;
            }
   ...
   public function store(Request $request)
{
    ...
    /* ===============================
       Datetime build
    =============================== */
    $checkIn  = Carbon::parse($request->booking_date.' '.$request->start_time);
    $checkOut = Carbon::parse($request->booking_date.' '.$request->end_time);

    /* ===============================
       Overlap protection (IMPORTANT)
    =============================== */
    ...
    if ($exists) {
        return back()->withErrors([
            'room_id' => 'This room is already booked for selected time.'
        ])->withInput();
    }
    ...
    $totalAmount = $totalHours * $room->price_per_hour;

    $paymentStatus = ($request->paid_amount >= $totalAmount) ? 1 : 0;
    ...
    /* ===============================
       Booking save
    =============================== */
    $checkOutDate = Carbon::parse($request->booking_date)->addDay();
    ...
    return redirect()->route('booking.create', [
        ...
        'message' => 'Hourly booking successful!',
        'alert-type' => 'success'
    ]);
}
    ...
    public function hourlySearch(Request $request)
{
    ...
    $checkIn  = Carbon::parse($request->date . ' ' . $request->start_time);
    $checkOut = Carbon::parse($request->date . ' ' . $request->end_time);
  ...
  $isBooked = Booking::where('room_id', $room->id)
    ->where(function ($q) use ($checkIn, $checkOut) {
        ...
        // 1️⃣ Hourly bookings
        $q->whereNotNull('check_in_datetime')
          ->whereNotNull('check_out_datetime')
          ->where(function ($sub) use ($checkIn, $checkOut) {
              $sub->where('check_in_datetime', '<', $checkOut)
                  ->where('check_out_datetime', '>', $checkIn);
          });

        // 2️⃣ Day-wise bookings (only where no datetime is set)
        $q->orWhere(function ($sub) use ($checkIn, $checkOut) {
            $sub->whereNull('check_in_datetime')
                ->whereNull('check_out_datetime')
