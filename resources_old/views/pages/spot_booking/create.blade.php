...
<div class="shad-card mb-6" id="spotPackagesCard">
    ...
    <div class="shad-body">
        ...
        <div class="total-row mt-5">
                ...
                <span class="fw-bold text-primary fs-4">৳ <span id="packageTotal">0</span></span>
            </div>
    </div>
</div>
            ...
            <div class="col-md-6">
                ...
                <div class="summary-box">
                    ...
                    <div class="summary-line">
                        ...
                        <span class="fw-semibold">৳ <span id="summaryPackage">0</span></span>
                    </div>
                ...
                <div class="row">
                    <div class="text-danger col-md-6" id="discountRow">
                        <span>Discount (<span id="discountLabel">0</span>%) </span><span> <input type="number"
                            id="discountPercent"
                            class="ps-0 ms-0 shad-input-sm1 text-end"
                            value="0"
                            min="0" max="{{ $discountLimit }}"></span>
                       
                    </div>
                   <div class="col-md-6 text-end">
                        <span>-৳ 
                            <span id="discountAmountText" style="cursor:pointer;">0</span>
...
function getSpotDiscountPercent(){
    ...
    return spotCount * MULTIPLE_SPOT_DISCOUNT;
}
...
function renderSpots(dateStr){
    ...
    spots.forEach(spot=>{
        const booked = bookedSpots[dateStr]?.includes(spot.id);
        box.insertAdjacentHTML('beforeend', `
            <div class="col-4">
                <button type="button"
                    class="pkg-btn spot-btn w-100 text-start ${booked?'disabled':''}"
                    data-id="${spot.id}" ${booked?'disabled':''}>
...
function renderSelected(){
    ...
    const discountedSpot = rawSpotTotal - (rawSpotTotal * spotDiscount / 100);
...
function updateSummary(){
    ...
    const spotTotal = rawSpot - (rawSpot * spotDisc / 100);
    ...
    document.getElementById('packageTotal').innerText = formatMoney(packageTotal);
    ...
    document.getElementById('grandTotal1').innerText = formatMoney(subtotal.toFixed(2));
    document.getElementById('grandTotal').innerText = formatMoney(grandTotal.toFixed(2));
...
function initManualDiscountEvents(){
    const text = document.getElementById('discountAmountText');
...
$('form').on('submit', function () {
    $('#spot_discount_percent').val(getSpotDiscountPercent());
});
